...
(function($) {
    ...
    function customSlidesPagination(container){
        var paginContainer = $(container).find('.fp-slidesNav, .tp-bullets');
    ...
    var revapi = $('.tp-banner').show().revolution(
	{
		delay:900000,
	...
	revapi.one('revolution.slide.onloaded', function() {
        customSlidesPagination('.tp-banner');
    });
    revapi.on('revolution.slide.onbeforeswap', function(event, data) {
	    var currentBullet = $('.tp-bullet').eq(data.nextslide.index());
	    var offset = currentBullet.position();
	    if(data.nextslide.index()+1 == 2){
        	$('.tp-bullets .tp-bullet, .pagination-line, .tparrows, #header').addClass('dark');
        } else {
        	$('.tp-bullets .tp-bullet, .pagination-line, .tparrows, #header').removeClass('dark');
        }
	    var paginationLineItem = $('.pagination-line-container .pagination-line .line-item');
    ...
    $('.special-hover span, .special-hover div, a.tp-caption h2, #lightbox-footer .socials li div').each(function(i,e){
        var e = $(e);
        e.divide_word();
    });
